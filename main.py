...
class AXIE_STATUS_Enum(Enum):
    Normal = 'normal'
    Fire = 'fire'
    Die = 'die'


AXIES_FORMATION = [{'id': '1986033', 'position_id': 6, 'status': AXIE_STATUS_Enum.Normal, 'status_count': 0},  # fish
                   {'id': '6805632', 'position_id': 3, 'status': AXIE_STATUS_Enum.Normal, 'status_count': 0},  # pink
                   {'id': '7035133', 'position_id': 0, 'status': AXIE_STATUS_Enum.Normal, 'status_count': 0}  # green
                   ]
...
AXIE_HEAL_RELATION = [{'id': "1986033", 'healby': '1985659', 'healcard': []},
                      {'id': "1980381", 'healby': '1980381', 'healcard': ['2-2.JPG']},
                      {'id': "6805632", 'healby': '1985659', 'healcard': []},
                      {'id': "7035133", 'healby': '1985659', 'healcard': ['7035133-2.JPG']}
                      ]
MY_AXIES = [
    ...
    {'id': "6805632", 'nickname': 'pink', 'health': 1393, 'speed': 91, 'skill': 64, 'morale': 8591,
     'x': 0, 'y': 0, 'debuff': [], 'healByWhichAxie': '1985659',
     'cards': [{'name': '6805632-1.JPG', 'attack': 241, 'shield': 161, 'energy': 1, 'heal': 0,
                'nickname': 'chomp', 'property': ['attack', 'shield'], 'other': 'heal front', 'comboed': True,
                'debuff_seq': 3, 'comboed_number': 2},
               {'name': '6805632-2.JPG', 'attack': 96, 'shield': 0, 'energy': 0, 'heal': 0,
                'nickname': 'mystic rush', 'property': ['attack'], 'other': 'heal front or itself', 'comboed': False,
                'debuff_seq': 1},
               {'name': '6805632-3.JPG', 'attack': 129, 'shield': 193, 'energy': 1, 'heal': 0,
                'nickname': 'sticky goo', 'property': ['attack', 'shield'], 'other': '', 'comboed': False,
                'debuff_seq': 0},
               {'name': '6805632-4.JPG', 'attack': 322, 'shield': 96, 'energy': 1, 'heal': 0,
                'nickname': 'allergic reaction', 'property': ['attack', 'shield'], 'other': '', 'comboed': False,
                'debuff_seq': 2},
               ]},
    {'id': "7035133", 'nickname': 'green', 'health': 1664, 'speed': 71, 'skill': 88, 'morale': 85,
     'x': 0, 'y': 0, 'debuff': [], 'healByWhichAxie': '1985659',
     'cards': [{'name': '7035133-1.JPG', 'attack': 96, 'shield': 96, 'energy': 1, 'heal': 0,
                'nickname': 'vegetal bite', 'property': ['attack', 'shield'], 'other': 'heal front', 'comboed': True,
                'debuff_seq': 1, 'comboed_number': 1},
               {'name': '7035133-2.JPG', 'attack': 0, 'shield': 161, 'energy': 1, 'heal': 387,
                'nickname': 'healing aroma', 'property': ['heal'], 'other': 'heal front or itself', 'comboed': False,
                'debuff_seq': 2},
               {'name': '7035133-3.JPG', 'attack': 145, 'shield': 258, 'energy': 1, 'heal': 0,
                'nickname': 'agua stock', 'property': ['attack', 'shield'], 'other': '', 'comboed': False,
                'debuff_seq': 0},
               {'name': '7035133-4.JPG', 'attack': 225, 'shield': 129, 'energy': 1, 'heal': 0,
                'nickname': 'carrot hammer', 'property': ['attack', 'shield'], 'other': '', 'comboed': False,
                'debuff_seq': 3},
               ]},
          ...
          'sub_mission':
              [{'mission_id': '19-0', 'monsters_number': 3,
               ...
               {'mission_id': '19-2', 'monsters_number': 3,
                'monster': [
                    {'id': 1, 'max_hp': 1025, 'property': '燈籠', 'hp_x': 948, 'hp_y': 328, 'cur_hp': 0, 'idx_x': 0,
                     'idx_y': 0},
                    {'id': 0, 'max_hp': 924, 'property': 'plant', 'hp_x': 837, 'hp_y': 391, 'cur_hp': 0, 'idx_x': 0,
                     'idx_y': 0},
                    {'id': 2, 'max_hp': 864, 'property': 'stone', 'hp_x': 948, 'hp_y': 490, 'cur_hp': 0, 'idx_x': 0,
                     'idx_y': 0}]}
               ]
...
def is_used(file_name):
    try:
        if not os.path.exists(file_name):
            print(file_name, 'not exist return false/')
            return False
...
def detect_cards():
    ...
    while count < 3:
        for f in CARDS_LIBRARY_LIST:
            ...
            idx, myaxie = axie_formation.value_index('id', card_id)
            if myaxie and myaxie['status'] != AXIE_STATUS_Enum.Normal:
                continue
...
def bounds_check(center_x, center_y):
    ...
    if bounds_degree <= 12:
        ...
        pg.click(center_x, center_y, duration=0.2)
        time.sleep(10)
        return True
    return False
...
def fight_result_check(center_x, center_y):
    ...
    if degree_win <= 3 or degree_lost <= 3:
        ...
        pg.moveTo(corner_x, corner_y, duration=0.2)
        # 點擊，離開 關卡說你勝利的畫面
        pg.click(corner_x, corner_y, duration=0.2)
        time.sleep(5)
...
def stratgy(runtime_cards_library, play_cards_array, axie_status_info, MISSION_ID, detect_debuffer=False):
    # 把卡片根據AXIE分類
    ...
    for sequ_axie in axies_sequence:
        ...
        if (the_pk_monster == None) or len(the_pk_monster) <= 0:
            print(sequ_axie, 'has no pk monster')
            continue
        ...
        for card in selected_attack_card_name:
            ...
            if len(play_cards_array[idx - 1]) < 4:
                ...
                # print("還有的卡牌:", runtime_cards_library_copy)
                # print(" 要打出去的卡:", card)
                runtime_cards_library_copy.remove(card)
            else:
                break
...
def detect_axie_status(axie_formation_global):
    axie_final_status = []
    answer = []
    for axies in axie_formation_global:
        axie_status = axies['status']
        axie_status_count = axies['status_count']
        # 如果axie被記錄超過3次以上不是normal 就直接忽略他，不抓他的資料了
        # 會設定3次是為了容錯率，因為有時候會誤判axie的status, 可能還是normal但我們卻覺得是die or fire
        if (axie_status != AXIE_STATUS_Enum.Normal) and (axie_status_count >= 3):
            continue
        axie_id = axies['id']
        ...
        final = nickname + " is " + AXIE_STATUS_Enum(AXIE_STATUS[min_index]).value
        axie_final_status.append(final)
        axies['status'] = AXIE_STATUS_Enum(AXIE_STATUS[min_index])
        if AXIE_STATUS_Enum(AXIE_STATUS[min_index]) != AXIE_STATUS_Enum.Normal:
            axies['status_count'] = axie_status_count + 1

        # a = {'id': axie_id, 'status': AXIE_STATUS[min_index], 'status_id': min_index}
        a = {'id': axie_id, 'status': AXIE_STATUS_Enum(AXIE_STATUS[min_index]).value}
        answer.append(a)
...
def collect_the_monster_sequence(monsters):
    ...
    for m in monsters:
        ...
        # 移除空的dict
    monster_sequence_copy = []
    for i in range(7):
        sequ_monster = monster_sequence[i]

        if len(sequ_monster) > 0:
            monster_sequence_copy.append(sequ_monster)
    return monster_sequence_copy
...
def detect_the_next_pk_monster(myaxie, sub_monsters):
    ...
    for i in range(len(monster_sequence)):
        sequ_monster = monster_sequence[i]
        if len(sequ_monster) > 0:
            curhp = sequ_monster['cur_hp']
            if curhp > 0:
                monster_sequence_copy.append(sequ_monster)

    monster_length = len(monster_sequence_copy)
    if monster_length <= 0:
        return None
    # 只剩最後一隻怪物的話，就不用判斷了
    if monster_length == 1:
        the_pk_monster = monster_sequence_copy[0]
        return the_pk_monster

    for i in range(monster_length):
        sequ_monster = monster_sequence_copy[i]
        ...
        # 如何可能有並排的怪物 那就要檢查一下
        if m_select_position['rule'] == POSITION_RUEL.DOUBLE:
            ...
            # 找下一個怪物的資料，看是不是並排的怪物
            if i < monster_length - 1:
                next_sequence_m = monster_sequence_copy[i + 1]
                next_m_col = None
                next_m_rule = None
...
def test2(a):
    # detect_the_next_pk_monster
    print(type(AXIE_STATUS_Enum('normal')))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        axie_status = detect_axie_status(AXIES_FORMATION)  # create runtime axies
        # print_list("status:", axie_status)
        print_list('global status:', AXIES_FORMATION)
        # exit()
